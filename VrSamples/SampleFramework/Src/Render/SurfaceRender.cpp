													 ...
													 const int eye )
{
	...
	for ( const ovrDrawSurface & drawSurface : surfaceList )
	{
		...
		if ( cmd.Program.IsValid() && cmd.Program.UseDeprecatedInterface == false )
		{
			...
			// Update globally defined system level uniforms.
			{
				...
				if ( cmd.Program.ProjectionMatrix.Location >= 0 )
				{
					...
					GL( glUniformMatrix4fv( cmd.Program.ProjectionMatrix.Location, GlProgram::MAX_VIEWS, GL_FALSE, projMatrixT[0].M[0] ) );
				}
				if ( cmd.Program.ViewMatrix.Location >= 0 )
				{
					...
					GL( glUniformMatrix4fv( cmd.Program.ViewMatrix.Location, GlProgram::MAX_VIEWS, GL_FALSE, viewMatrixT[0].M[0] ) );
				}
				// ----IMAGE_EXTERNAL_WORKAROUND
			}
		...
		else // ----DEPRECATED_GLPROGRAM
		{
			...
			assert( cmd.numUniformTextures <= ovrUniform::MAX_UNIFORMS );
			for ( int textureNum = 0; textureNum < cmd.numUniformTextures; textureNum++ )
			{
				...
				if ( currentTextures[textureNum] != texNObj )
				{
					...
					GL( glBindTexture( cmd.uniformTextures[textureNum].target ?
...

			{
				...
				assert( cmd.Program.Program != 0 );
				if ( cmd.Program.Program != currentProgramObject )
				{
					counters.numProgramBinds++;

					currentProgramObject = cmd.Program.Program;
					GL( glUseProgram( currentProgramObject ) );
				}
			}
			...
			// Update the program parameters
			{
				...
				{
					GL( glUniformMatrix4fv( cmd.Program.uMvp, 1, GL_TRUE, &mvp.M[0][0] ) );
				}

				// set the model matrix
				if ( cmd.Program.uModel != -1 )
				{
					GL( glUniformMatrix4fv( cmd.Program.uModel, 1, GL_TRUE, &drawSurface.modelMatrix.M[0][0] ) );
				}

				// set the joint matrices ubo
				if ( cmd.Program.uJoints != -1 )
				{
					assert( cmd.Program.uJointsBinding != -1 );
					...
					if ( currentBuffers[cmd.Program.uJointsBinding] != bufferObj )
					{
						...
						GL( glBindBufferBase( GL_UNIFORM_BUFFER, cmd.Program.uJointsBinding, bufferObj ) );
					}
				}
			}

			{
				for ( int unif = 0; unif < ovrUniform::MAX_UNIFORMS; unif++ )
				{
					...
					counters.numParameterUpdates++;
					GL( glUniform4fv( slot, 1, cmd.uniformValues[unif] ) );
				}
			}
		}	// ----DEPRECATED_GLPROGRAM
