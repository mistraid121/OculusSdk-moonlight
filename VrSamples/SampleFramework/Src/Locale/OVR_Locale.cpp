...
bool ovrLocaleInternal::GetStringJNI( char const * key, char const * defaultOut, std::string & out ) const
{
	...
	jobject idObject = jni.CallObjectMethod( resources, getIdentifierMethod, keyObj, stringObj, packageName ); 
	if ( idObject == NULL )
	{
		ALOG( "GetStringJNI idObject == NULL" );
		out = "JAVAERROR";
		return false;
	}
	...
	if ( intClass == NULL )
	{
		ALOG( "GetStringJNI intClass == NULL" );
		out = "JAVAERROR";
		return false;
	}

    jmethodID intValueMethod = jni.GetMethodID( intClass, "intValue", "()I");
	if ( intValueMethod == NULL )
	{
		ALOG( "GetStringJNI intValueMethod == NULL" );
		out = "JAVAERROR";
		return false;
	}

	int id = jni.CallIntMethod( idObject, intValueMethod ); 
	if ( id == 0 )
	{
	...
	jobject textObject = jni.CallObjectMethod( resources, getTextMethod, idObject ); 
	if ( textObject == NULL )
	{
